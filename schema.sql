...
create table "mailkiteMailbox" ("id" text not null primary key, "address" text not null unique, "userId" text references "user" ("id") on delete cascade, "organizationId" text, "domainId" text, "createdAt" date not null);

create table "mailkiteMessage" ("id" text not null primary key, "mailboxId" text not null references "mailkiteMailbox" ("id") on delete cascade, "messageId" text not null unique, "fromAddress" text not null, "toAddress" text not null, "subject" text, "text" text, "html" text, "threadId" text, "read" integer, "receivedAt" date not null);
...
create unique index "mailkiteMailbox_address_uidx" on "mailkiteMailbox" ("address");

create index "mailkiteMailbox_userId_idx" on "mailkiteMailbox" ("userId");

create index "mailkiteMailbox_organizationId_idx" on "mailkiteMailbox" ("organizationId");

create index "mailkiteMessage_mailboxId_idx" on "mailkiteMessage" ("mailboxId");

create unique index "mailkiteMessage_messageId_uidx" on "mailkiteMessage" ("messageId");

create index "mailkiteMessage_threadId_idx" on "mailkiteMessage" ("threadId");